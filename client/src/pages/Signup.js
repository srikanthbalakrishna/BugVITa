...
import "../pages/InputValidation.css";
function Signup(props) {
  const [user, setUser] = useState();
  const [pass, setPass] = useState();
  const [validState, setValidState] = useState({ email: "", password: "" });

  function handleUserChange(event) {
    const regex = new RegExp("^[A-Za-z0-9+_.-]+@(.+)$"); //single @ symbol
    if (regex.test(event.target.value))
      setValidState({ ...validState, email: "" });
    else setValidState({ ...validState, email: "invalid" });
    setUser(event.target.value);
  }

  function handlePassChange(event) {
    const regex = new RegExp("(?=.*?[A-Z])"); //atleast 1 uppercase letter
    if (regex.test(event.target.value))
      setValidState({ ...validState, password: "" });
    else setValidState({ ...validState, password: "invalid" });
  ...
  function handleClick(e) {
    ...
    axios
      .post("http://localhost:4000/api/users/signup", reqbody)
  ...
  function handleResponse(res) {
    console.log(res);
    if (res.data.signedup) window.location.href = "/dashboard";
    //   else
            ...
            <form class="mt-6" action="#" method="POST">
              <div>
                ...
                <input
                  ...
                  class={
                    "w-full px-4 py-2 mt-2 text-base text-black transition duration-500 ease-in-out transform bg-gray-100 border-transparent rounded-lg focus:border-gray-500 focus:bg-white focus:outline-none focus:shadow-outline focus:ring-2 ring-offset-current ring-offset-2 " +
                    validState.email
                  }
                  autofocus
                  autocomplete
              ...
              <div class="mt-4">
                ...
                <input
                  ...
                  class={
                    "w-full px-4 py-2 text-base text-black transition duration-500 ease-in-out transform bg-gray-100 border-transparent rounded-lg focus:border-gray-500 focus:bg-white focus:outline-none focus:shadow-outline focus:ring-2 ring-offset-current ring-offset-2 " +
                    validState.password
                  }
                  required
                />
